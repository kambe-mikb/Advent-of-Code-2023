...
Your puzzle answer was 250898830.
...
Your puzzle answer was 252127335.
"""
...
ThreeOfAKind = Rank.ThreeOfAKind
FullHouse = Rank.FullHouse
FourOfAKind = Rank.FourOfAKind
FiveOfAKind = Rank.FiveOfAKind


def getHand(input: T.Iterable) -> tuple[str, int]:
    for line in input:
        cards, bid = line.split()
        yield cards, int(bid)


def rankHandSort(cardOrder: str, handTuple: tuple[str, int]) -> int:
    hand = handTuple[0]
    handSet = frozenset(hand)
    freq = [(card, hand.count(card)) for card in frozenset(hand)]
    freq.sort(key=lambda x: cardOrder.index(x[0]), reverse=True)
    freq.sort(key=itemgetter(1), reverse=True)
    if cardOrder.startswith("J") and "J" in handSet and len(handSet) > 1:
        # Using Jokers, not Jacks
        jFreq = hand.count("J")
        freq.remove(("J", jFreq)) 
        hand2 = hand.replace("J", freq[0][0])
        # Use our "upgraded" hand
        handSet = frozenset(hand2)
        freq = [(card, hand2.count(card)) for card in handSet]
        freq.sort(key=itemgetter(1), reverse=True)
    if len(handSet) == 1: # Five of a kind => 1 unique card
        rank = FiveOfAKind
    elif len(handSet) == 2: # Four of a Kind or Full House
        rank = FourOfAKind if freq[0][1] == 4 else FullHouse
    elif len(handSet) == 3: # Three of a Kind or Two Pair
        rank = ThreeOfAKind if freq[0][1] == 3 else TwoPair
    elif len(handSet) == 4: # One Pair
        rank = OnePair
    else: # Highest Card
        rank = HighCard
    return reduce(lambda x, y: x * 13 + cardOrder.index(y), hand, rank.value)


# The solution doesn't change between parts 1 and 2, only the ranking (sort
# order)
def rankHands(cardOrder: str, input: T.Iterable) -> int:
    rankHand = partial(rankHandSort, cardOrder)  # customise key function
    rankedHands = [handTuple for handTuple in getHand(input)]
    rankedHands.sort(key=rankHand)  # sort in place - no value returned
    return reduce(
        lambda x, y: x + (y[0] * y[1][1]), enumerate(rankedHands, start=1), 0
    )
    

# Solution with Jacks
part_1 = partial(rankHands, "23456789TJQKA")
# Solution with Jokers
part_2 = partial(rankHands, "J23456789TQKA")


if __name__ == "__main__":
    puzzle_input = "Input07.txt"
