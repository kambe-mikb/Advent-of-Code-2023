...
line_regex = compile(r"\d+")


def getMetrics(input: T.Iterable) -> list[tuple[int, int]]:
    data = []
    for line in input:
        data.append([int(value[0]) for value in line_regex.finditer(line)])
            ...
            len(
                [
                    ...
                    for travel in filterfalse(
                        lambda wait: (wait * (time - wait)) <= distance,
                        range(time),
                    )
                ]
            )
            for time, distance in getMetrics(input)
        ]
    )
