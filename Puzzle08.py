...
vertical bar (|): a list of winning numbers and then a list of numbers you have. You organize the information into a table (your puzzle input).
...
Card = namedtuple("Card", ["card", "winners", "numbers"])


def get_numbers(input: T.Iterable) -> list[tuple[str, tuple[str], tuple[str]]]:
    cards = []
    for line in input:
        (card_id, card_details) = line.split(": ")
        (winners, holdings) = card_details.split(" | ")
        cards.append(Card(card_id, winners.split(), holdings.split()))
    return cards
...
def part_2(input: T.Iterable) -> int:
    ...
    for card in active_cards:
        ...
        if winners:
            ...
            active_cards.extend(cards[index : (index + winners)])
    return len(active_cards)


if __name__ == "__main__":
    puzzle_input = "Input04.txt"
