    ...
    for line in input:
        ...
        for game in games.split("; "):
            for cubes in game.strip().split(", "):
                pulls, colour = cubes.strip().split()
                pull = int(pulls)
                unfeasible = pull > maximum[colour] 
                if unfeasible:
                    print(f"{id} game {game} is not feasible")
                    break
            if unfeasible:
                break
        else:
            print(f"{id} is feasible")
...
def part_2(input: T.Iterable) -> int:
    ...
    for line in input:
        id, games = line.split(": ")
        pool = dict(zip(maximum.keys(), repeat(0)))
        for game in games.split("; "):
            for cubes in game.split(", "):
                pulls, colour = cubes.split()
                pull = int(pulls)
                pool[colour] = max(pool[colour], pull)
        pool_product = prod(pool.values())
        print(f"{line} => {pool_product}")
        total += prod(pool.values())
    return total
...
if __name__ == "__main__":
    ...
    print()
    print(f"Result of Part 1 (data) = {part_1(getInput('Input02.txt'))}")
    print()
    print(f"Result of Part 2 (test) = {part_2(input)}")
    print()
    print(f"Result of Part 1 (data) = {part_2(getInput('Input02.txt'))}")
