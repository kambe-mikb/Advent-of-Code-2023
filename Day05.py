...
Your puzzle answer was 31599214."""
...
MapRange = namedtuple("MapRange", ["dest", "source_lower", "source_length"])


# We cache the result because it doesn't change between runs
def get_almanac(input: T.Iterable) -> dict:
    almanac = {}
    lists = []
    for line in input:
        if line.startswith("seeds: "):
            words = line.split()
            key = words.pop(0).rstrip(": ")
            almanac[key] = [int(word) for word in words]
        elif line.endswith(":"):
            key = line.split()[0]
        elif line:
            lists.append(MapRange(*[int(word) for word in line.split()]))
        else:
            if key not in almanac:
                almanac[key] = lists
            lists = []
    else:
        if key not in almanac:
...
def map_number(mapping: list[MapRange], source: int) -> int:
    for line in mapping:
        if (
            line.source_lower
            <= source
            <= (line.source_lower + line.source_length - 1)
        ):
            return line.dest + (source - line.source_lower)
    else:
        return source
