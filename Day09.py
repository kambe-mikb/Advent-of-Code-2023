...
def getHistory(input: T.Iterable) -> list[list[int]]:
    for line in input:
...
def part_1(input: T.Iterable) -> int:
    total = 0
    for line in getHistory(input):
        diff = [line,]
        while any([x for x in diff[-1]]):
            diff.append([n2 - n1 for n1, n2 in pairwise(diff[-1])])
        diff.reverse()
        running_extrapolation = 0
        for extrapolation in diff:
            running_extrapolation += extrapolation[-1]
        total += running_extrapolation
    return total
...
def part_2(input: T.Iterable) -> int:
    ...
    for line in getHistory(input):
        ...
        running_extrapolation = 0
        for extrapolation in diff:
            running_extrapolation = extrapolation[0] - running_extrapolation
        total += running_extrapolation
    return total


if __name__ == "__main__":
    puzzle_input = "Input09.txt"
